...
import { HTTPCallViewer } from './HTTPCallViewer'

export function SessionDetail() {
  const { sessionId } = useParams<{ sessionId: string }>()
  
  console.log('SessionDetail rendered with sessionId:', sessionId)
  ...
  console.log('SessionDetail state:', { isLoading, error, hasData: !!session })
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {session.llm_calls.map((call) => (
          <LLMCallViewer key={call.id} call={call} />
        ))}
        
        {session.tool_calls.map((call) => (
          <ToolCallViewer key={call.id} call={call} />
        ))}
        
        {session.http_calls.map((call) => (
          <HTTPCallViewer key={call.id} call={call} />
        ))}
      </div>
    </div>
  )
